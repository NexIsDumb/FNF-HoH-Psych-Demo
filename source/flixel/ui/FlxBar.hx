...
class FlxBar extends FlxSprite {
	...
	function set_fillDirection(direction:FlxBarFillDirection):FlxBarFillDirection {
		...
		switch (direction) {
			case LEFT_TO_RIGHT, RIGHT_TO_LEFT, HORIZONTAL_INSIDE_OUT, HORIZONTAL_OUTSIDE_IN: _fillHorizontal = true;

			case TOP_TO_BOTTOM, BOTTOM_TO_TOP, VERTICAL_INSIDE_OUT, VERTICAL_OUTSIDE_IN: _fillHorizontal = false;
		}
	...
	public function updateFilledBar():Void {
		...
		if (percent > 0) {
			switch (fillDirection) {
				case LEFT_TO_RIGHT, TOP_TO_BOTTOM: //	Already handled above
