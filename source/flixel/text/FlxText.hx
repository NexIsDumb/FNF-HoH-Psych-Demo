...
class FlxText extends FlxSprite {
	 ...
	 */
	public function setFormat(?Font:String, Size:Int = 8, Color:FlxColor = FlxColor.WHITE, ?Alignment:FlxTextAlign, ?BorderStyle:FlxTextBorderStyle, BorderColor:FlxColor = FlxColor.TRANSPARENT, EmbeddedFont:Bool = true):FlxText {
	...
	var defaultLang:Array<String> = [];

	public var testLang:Array<String> = [];
	public var diaLang:Array<String> = [];
	public var defLang:Array<String> = [];

	function splitText(path:String):Array<String> {
	...
	function splitTextCharm(path:String):Array<String> {
		var daList:Array<String> = Assets.getText(path).trim().split('\n');

		for (i in 0...daList.length) {
			testLang.insert(testLang.length, daList[i]);
		}

		return daList;
	}

	function splitDefTextCharm(path:String):Array<String> {
		var daList:Array<String> = Assets.getText(path).trim().split('\n');

		for (i in 0...daList.length) {
			defaultLang.insert(defaultLang.length, daList[i].trim());
		}
	...
	public function splitlang() {
		if (curlang != ClientPrefs.getLanguage()) {
			defaultLang = splitText("assets/language/eng_menu.lang");
			splitDefTextCharm("assets/language/eng_charms.lang");
			testLang = splitText("assets/language/" + ClientPrefs.getLanguage() + "_menu.lang");
			splitTextCharm("assets/language/" + ClientPrefs.getLanguage() + "_charms.lang");
			diaLang = splitText("assets/language/" + ClientPrefs.getLanguage() + "_dialog.lang");
			defLang = splitText("assets/language/eng_dialog.lang");
			curlang = ClientPrefs.getLanguage();
		}
	}
	...
	var basetext:String = "";
	var customfonts:Array<String> = ['chi', 'ukr', 'rus', 'jap'];
	...
	public function translationPub(stringy:String) {
		...
		var suffix:String = "";
		for (i in 0...customfonts.length) {
			var thing = customfonts[i];
			if (thing == ClientPrefs.getLanguage()) {
				thebool = true;
			}
		}
		if (silly > -1 && thebool) {
			Text = testLang[silly];
			suffix = "[" + ClientPrefs.getLanguage() + "]";
		} else {
			if (silly > -1) {
				Text = testLang[silly];
			}
	...
	public function suffixPub(string:String) {
		splitlang();
		var silly:Int = defaultLang.indexOf(string);
		var thebool:Bool = false;
	...
	public function translation(sui:String) {
		splitlang();
		var Text = sui;
		var suffix:String = '';
		if (Text.endsWith(":")) {
		...
		var silly:Int = defaultLang.indexOf(Text);
		var thebool:Bool = false;
		for (i in 0...customfonts.length) {
			var thing = customfonts[i];
			if (thing == ClientPrefs.getLanguage()) {
		...
		if (silly > -1 && thebool) {
			textField.text = testLang[silly] + "[" + ClientPrefs.getLanguage() + "]";
			Text = textField.text;
		} else {
			if (silly > -1) {
				textField.text = testLang[silly];
				Text = textField.text;
			}
		}

		_defaultFormat.size = basesize;
		if (basefont == "Perpetua" || basefont == "Trajan Pro") {
	...
	public function dialog(sui:String) {
		var silllyyyyy = sui;
		var suffix:String = '';
		if (silllyyyyy.endsWith(":")) {
			suffix = ":";
			silllyyyyy = silllyyyyy.replace(":", "");
			silllyyyyy = silllyyyyy.replace(":", "");
			silllyyyyy = silllyyyyy.replace(":", "");
		}

		var silly:Int = defLang.indexOf(silllyyyyy);
		var thebool:Bool = false;
		for (i in 0...customfonts.length) {
			var thing = customfonts[i];
			if (thing == ClientPrefs.getLanguage()) {
		...
		var result:String = sui;
		if (silly > -1 && thebool) {
			result = diaLang[silly] + "[" + ClientPrefs.getLanguage() + "]";
		} else {
			if (silly > -1) {
				result = diaLang[silly];
			}
		}
	...
	function regenGraphic(?def:TextFormat):Void {
		if (def == null) {
			def = _defaultFormat;
		}
		...
		if (textField != null && textField.text != null) {
			...
			copyTextFormat(def, _formatAdjusted);
	...
	function getthefont(key:String) {
