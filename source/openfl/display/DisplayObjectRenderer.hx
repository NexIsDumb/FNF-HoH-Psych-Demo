...
class DisplayObjectRenderer extends EventDispatcher {
	...
	@:noCompletion private function __renderEvent(displayObject:DisplayObject):Void {
		...
		if (displayObject.__customRenderEvent != null && displayObject.__renderable) {
			...
			switch (renderer.__type) {
				...
				case CAIRO: displayObject.__customRenderEvent.type = RenderEvent.RENDER_CAIRO;
				...
				case CANVAS: displayObject.__customRenderEvent.type = RenderEvent.RENDER_CANVAS;
	...
	@:noCompletion private function __shouldCacheHardware(displayObject:DisplayObject, value:Null<Bool>):Null<Bool> {
		...
		switch (displayObject.__drawableType) {
			...
			case TEXT_FIELD: return value == true ? true : false;

			case TILEMAP: return true;

			default: return __shouldCacheHardware_DisplayObject(displayObject, value);
		}
	}
	...
	@:noCompletion private function __updateCacheBitmap(displayObject:DisplayObject, force:Bool):Bool {
		...
		switch (displayObject.__drawableType) {
			case BITMAP:
				...
				if (bitmap.__bitmapData == null || (bitmap.__filters == null #if lime && renderer.__type == OPENGL #end && bitmap.__cacheBitmap == null))
					return false;
			...
			case TEXT_FIELD:
				...
				if (textField.__filters == null #if lime && renderer.__type == OPENGL #end && textField.__cacheBitmap == null && !textField.__domRender)
					return false;
				if (force)
		...
		if (displayObject.cacheAsBitmap || (renderer.__type != OPENGL && !colorTransform.__isDefault(true)) || (renderer.__type == OPENGL && displayObject.scale9Grid != null)) {
			var rect = null;
